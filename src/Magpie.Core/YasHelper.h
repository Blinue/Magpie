...
struct serializer<
	...
	ser_case::use_internal_serializer,
	F,
	Magpie::Core::SmallVector<T, N>
> {
	template<typename Archive>
	static Archive& save(Archive& ar, const Magpie::Core::SmallVector<T, N>& vector) noexcept {
		return concepts::array::save<F>(ar, vector);
	}

	template<typename Archive>
	static Archive& load(Archive& ar, Magpie::Core::SmallVector<T, N>& vector) noexcept {
