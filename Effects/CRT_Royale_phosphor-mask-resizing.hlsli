	...
	const bool vertical)
{
	...
	const float2 first_texel = prev_texel - (samples/2.0 - 1.0);
	...
	const float2 coord_negative =
		float2(first_texel_tile_uv_wrap_2D < 0.0);
...
inline float4 tex2Dlod0try(Texture2D tex, SamplerState sam, const float2 tex_uv)
{
	...
	//  One [slow] workaround is to select the lowest mip level:
		...
		return tex.SampleLevel(sam, tex_uv, 0.0);
	#else
		#ifdef ANISOTROPIC_RESAMPLING_COMPAT_TEX2DBIAS
			return tex.SampleBias(sam, tex_uv, -16.0);
		#else
			return tex.Sample(sam, tex_uv);
		#endif
	#endif
}
...
//  We vectorize the loop to help reduce the Lanczos window's cost.
	...
	#define CALCULATE_R_COORD_FOR_4_SAMPLES                                    \
		const float4 true_i = i_base + i + float4(0.0, 1.0, 2.0, 3.0); \
		...
		#define CALCULATE_SINC_RESAMPLE_WEIGHTS                                \
			...
			const float4 weights = min(sin(pi_dist) * sin(pi_dist_over_lobes) /\
				(pi_dist*pi_dist_over_lobes), 1.0);
	#else
		#define CALCULATE_SINC_RESAMPLE_WEIGHTS                                \
			const float4 weights = min(sin(pi_dist)/pi_dist, 1.0);
	#endif
	...
	#define VERTICAL_SINC_RESAMPLE_LOOP_BODY                                   \
		...
		const float3 new_sample0 = tex2Dlod0try(tex, sam,                      \
			float2(tex_uv.x, tex_uv_r.x)).rgb;                                 \
		const float3 new_sample1 = tex2Dlod0try(tex, sam,                      \
			float2(tex_uv.x, tex_uv_r.y)).rgb;                                 \
		const float3 new_sample2 = tex2Dlod0try(tex, sam,                      \
			float2(tex_uv.x, tex_uv_r.z)).rgb;                                 \
		const float3 new_sample3 = tex2Dlod0try(tex, sam,                      \
...
float3 downsample_vertical_sinc_tiled(Texture2D tex, SamplerState sam,
	...
	const float magnification_scale, const float tile_size_uv_r)
{
	...
	const float3 scalar_weight_sum = weight_sum_reduce.x + weight_sum_reduce.y;
	return (pixel_color/scalar_weight_sum);
}

float3 downsample_horizontal_sinc_tiled(Texture2D tex, SamplerState sam,
	const float2 tex_uv, const float2 texture_size, const float dr,
	const float magnification_scale, const float tile_size_uv_r)
{
	...
	const bool solemnly_swear_same_inputs_for_every_pass)
{
	...
	const float2 final_resized_tile_size =
		floor(reclamped_tile_size + FIX_ZERO(0.0));
	...
	float2 duv_dx, float2 duv_dy)
{
	...
	const float2 pixel_in_first_half_tile = float2(tile_uv < 0.5);
	const float2 jump_exists = float2(duv_dx + duv_dy > 0.5);
