...
void Pass7(uint2 blockStart, uint3 threadId) {
	...
	for (i = 0; i <= 2; i += 2) {
		...
		for (j = 0; j <= 2; j += 2) {
			float2 tpos = (gxy + uint2(i, j)) * inputPt;
			const float4 sr = tex6.GatherRed(sam, tpos);
			const float4 sg = tex6.GatherGreen(sam, tpos);
			const float4 sb = tex6.GatherBlue(sam, tpos);
			const float4 sa = tex6.GatherAlpha(sam, tpos);

			// w z
			// x y
			src6[i][j] = float4(sr.w, sg.w, sb.w, sa.w);
			src6[i][j + 1] = float4(sr.x, sg.x, sb.x, sa.x);
			src6[i + 1][j] = float4(sr.z, sg.z, sb.z, sa.z);
			src6[i + 1][j + 1] = float4(sr.y, sg.y, sb.y, sa.y);
		}
	}
	...
	float4 src1[2][2];
	sr = tex1.GatherRed(sam, tpos);
	sg = tex1.GatherGreen(sam, tpos);
	sb = tex1.GatherBlue(sam, tpos);
	float4 sa = tex1.GatherAlpha(sam, tpos);

	src1[0][0] = float4(sr.w, sg.w, sb.w, sa.w);
	src1[0][1] = float4(sr.x, sg.x, sb.x, sa.x);
	src1[1][0] = float4(sr.z, sg.z, sb.z, sa.z);
	src1[1][1] = float4(sr.y, sg.y, sb.y, sa.y);

	float4 src2[2][2];
	sr = tex2.GatherRed(sam, tpos);
	sg = tex2.GatherGreen(sam, tpos);
	sb = tex2.GatherBlue(sam, tpos);
	sa = tex2.GatherAlpha(sam, tpos);

	src2[0][0] = float4(sr.w, sg.w, sb.w, sa.w);
	src2[0][1] = float4(sr.x, sg.x, sb.x, sa.x);
	src2[1][0] = float4(sr.z, sg.z, sb.z, sa.z);
	src2[1][1] = float4(sr.y, sg.y, sb.y, sa.y);

	float4 src3[2][2];
	sr = tex3.GatherRed(sam, tpos);
	sg = tex3.GatherGreen(sam, tpos);
	sb = tex3.GatherBlue(sam, tpos);
	sa = tex3.GatherAlpha(sam, tpos);

	src3[0][0] = float4(sr.w, sg.w, sb.w, sa.w);
	src3[0][1] = float4(sr.x, sg.x, sb.x, sa.x);
	src3[1][0] = float4(sr.z, sg.z, sb.z, sa.z);
	src3[1][1] = float4(sr.y, sg.y, sb.y, sa.y);
	...
	src4[0][0] = float4(sr.w, sg.w, sb.w, sa.w);
	src4[0][1] = float4(sr.x, sg.x, sb.x, sa.x);
	src4[1][0] = float4(sr.z, sg.z, sb.z, sa.z);
	src4[1][1] = float4(sr.y, sg.y, sb.y, sa.y);

	float4 src5[2][2];
	sr = tex5.GatherRed(sam, tpos);
	sg = tex5.GatherGreen(sam, tpos);
	sb = tex5.GatherBlue(sam, tpos);
	sa = tex5.GatherAlpha(sam, tpos);

	src5[0][0] = float4(sr.w, sg.w, sb.w, sa.w);
	src5[0][1] = float4(sr.x, sg.x, sb.x, sa.x);
	src5[1][0] = float4(sr.z, sg.z, sb.z, sa.z);
	src5[1][1] = float4(sr.y, sg.y, sb.y, sa.y);
	...
	for (i = 1; i <= 2; ++i) {
		...
		for (j = 1; j <= 2; ++j) {
			...
			float4 src7 = mul(max(src6[i - 1][j - 1], 0), float4x4(-0.35835463, 0.038305778, -0.10198824, -0.021951782, 0.02142098, -0.072417736, -0.2577152, 0.054713376, 0.075116105, -0.21191697, -0.1213158, -0.105036296, 0.12030758, -0.17591658, 0.1726511, 0.17754573));
			src7 += mul(max(src6[i - 1][j], 0), float4x4(0.32325825, 0.19869742, 0.333873, 0.39670366, 0.20716824, 0.09557955, 0.120742686, -0.2271023, 0.37509173, -0.031341635, 0.10247365, 0.031520665, -0.092765376, -0.13535516, 0.8333728, 0.05886494));
			src7 += mul(max(src6[i - 1][j + 1], 0), float4x4(-0.17573749, 0.16768494, 0.021141645, 0.19668253, 0.21080776, 0.31503728, -0.26834, 0.19103156, 0.21946241, 0.14559007, -0.09761235, -0.23565038, -0.49393657, -0.5332298, 0.09806347, 0.054431103));
			src7 += mul(max(src6[i][j - 1], 0), float4x4(-0.042109374, -0.05564321, 0.27586877, 0.010382545, 0.007322007, 0.13193823, -0.18262729, 0.06399193, 0.14174329, 0.3898842, -0.10398105, 0.01846146, -0.24542394, -0.13020967, -0.16491668, -0.03544872));
			src7 += mul(max(src6[i][j], 0), float4x4(-0.15291597, 0.1566557, 0.14745249, -0.23258151, 0.17843612, 0.15885495, -0.691466, -0.41177312, 0.40330106, -0.07991953, 0.2832403, 0.10656986, -0.19571523, 0.3670614, -0.62296015, -0.5666968));
			src7 += mul(max(src6[i][j + 1], 0), float4x4(-0.17513512, 0.011393021, -0.44352317, -0.059153114, -0.2227142, -0.033094753, 0.09624524, 0.051315393, 0.2632246, 0.09945105, 0.042561427, -0.1234722, 0.23755905, -0.506999, 0.114180565, 0.27887583));
			src7 += mul(max(src6[i + 1][j - 1], 0), float4x4(-0.459564, -0.120326266, 0.17507194, 0.06701153, -0.14124362, -0.36653697, -0.2856802, -0.22955593, -0.08515889, 0.18788262, 0.23427077, 0.021544341, 0.31996533, -0.2668834, 0.08469808, -0.01347926));
			src7 += mul(max(src6[i + 1][j], 0), float4x4(-0.14092083, -0.31244513, -0.044023518, 0.013948701, 0.33119613, -0.011959397, -0.1494438, -0.111066826, -0.11994278, 0.116068155, -0.13032633, -0.037004936, 0.13851176, -0.006655432, -0.39841232, -0.079951204));
			src7 += mul(max(src6[i + 1][j + 1], 0), float4x4(-0.08959123, 0.18297827, -0.0763483, 0.11364159, -0.04361797, -0.029816678, -0.19314721, -0.03484794, 0.044681285, 0.04669291, -0.30017474, -0.07453036, 0.090825416, -0.27414632, 0.36355078, 0.15742934));
			float3 result = mul(max(src1[i - 1][j - 1], 0), float4x3(-0.030150581, -0.002168429, 0.014918388, 0.020940892, 0.04591048, 0.049137186, 0.111167125, 0.05311203, 0.0625381, 0.020043287, 0.04785493, 0.040921766));
			result += mul(max(-src1[i - 1][j - 1], 0), float4x3(0.04158565, -0.008488135, 0.0020472286, 0.049123142, -0.055042226, -0.06489915, 0.09238876, 0.10387972, 0.09576964, -0.054776173, -0.098954335, -0.09018853));
			result += mul(max(src2[i - 1][j - 1], 0), float4x3(0.2081418, 0.08273068, 0.040325668, -0.09937802, -0.13162258, -0.13989717, -0.13983749, 0.01309777, 0.0023888077, -0.18937743, -0.07021057, -0.047152344));
			result += mul(max(-src2[i - 1][j - 1], 0), float4x3(-0.09646629, 0.080605574, 0.10463501, 0.22579835, 0.24077554, 0.22600271, 0.049726978, 0.015292378, -0.0047161994, 0.16281025, 0.048491795, 0.038338162));
			result += mul(max(src3[i - 1][j - 1], 0), float4x3(-0.09772107, -0.043998875, -0.054745924, -0.1257736, -0.13175423, -0.10889618, -0.015900036, 0.07074481, 0.08210496, -0.11321135, -0.12526917, -0.105605066));
			result += mul(max(-src3[i - 1][j - 1], 0), float4x3(0.14187162, 0.14032297, 0.13016908, 0.018954534, 0.016011704, 0.010169183, 0.04762765, -0.044460997, -0.06499567, 0.11133751, 0.09464176, 0.08865274));
			result += mul(max(src4[i - 1][j - 1], 0), float4x3(-0.16567162, -0.1744712, -0.1637222, -0.02412003, 0.0074480795, 0.007903436, -0.06161098, -0.046788957, -0.03971239, 0.030736001, 0.036460854, 0.03660504));
			result += mul(max(-src4[i - 1][j - 1], 0), float4x3(0.084027, 0.10024112, 0.08152756, 0.005087354, -0.026047802, -0.027264625, 0.10519243, 0.08977278, 0.077558964, -0.052826345, -0.06602686, -0.055083472));
			result += mul(max(src5[i - 1][j - 1], 0), float4x3(0.007862721, 0.009936555, 0.012004831, -0.042322706, -0.061728776, -0.05359773, 0.030532641, 0.045623366, 0.04214089, 0.030569768, 0.011892851, 0.0074041556));
			result += mul(max(-src5[i - 1][j - 1], 0), float4x3(0.03948997, 0.043119986, 0.039943404, 0.0526772, 0.06820589, 0.058139592, -0.062081397, -0.06755701, -0.054816127, -0.004076369, 0.0061744447, 0.016273081));
			result += mul(max(src6[i][j], 0), float4x3(0.0071622543, 0.004829105, -0.002032197, -0.048541367, -0.059043564, -0.05662218, 0.0015553127, 0.009178359, 0.009577062, 0.114169896, 0.1349016, 0.11432262));
			result += mul(max(-src6[i][j], 0), float4x3(0.019324556, 0.028323999, 0.027396113, 0.016746879, 0.01608199, 0.026891617, 0.12068619, 0.13617857, 0.113496214, -0.013930715, -0.014250072, -0.00824306));
			...
			WriteToOutput(destPos, result + origin[i - 1][j - 1]);
		}
	}
}
