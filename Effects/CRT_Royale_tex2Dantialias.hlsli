...
/////////////////////////////  SETTINGS MANAGEMENT  ////////////////////////////
	...
	static const float2 aa_xy_axis_importance =
		aa_filter < 5.5 ? float2(1.0, 1.0) :         //  Box, tent, Gaussian
...
static float aa_cubic_b;
static float cubic_branch1_x3_coeff;
static float cubic_branch1_x2_coeff;
static float cubic_branch1_x0_coeff;
static float cubic_branch2_x3_coeff;
static float cubic_branch2_x2_coeff;
static float cubic_branch2_x1_coeff;
static float cubic_branch2_x0_coeff;
#endif
...
float4 get_subpixel_support_diam_and_final_axis_importance()
{
	...
	else
	{
		...
		const float2 subpixel_support_radius = max(aa_box_support,
...
{
	return float(all(abs(offset) <= aa_box_support));
}
...
float4 tex2Daa_tiled_linearize(Texture2D tex, SamplerState sam, const float2 s)
{
	...
	//  confused.  This is one workaround:
		...
		return tex2Dlod_linearize(tex, sam, float4(s, 0.0, 0.0));
	#else
		return tex2D_linearize(tex, sam, s);
	#endif
}
...
float2 get_frame_sign(const float frame)
{
	if(aa_temporal)
	{
		...
		const float2 aa_r_offset = get_aa_subpixel_r_offset();
		const float2 mirror = -float2(abs(aa_r_offset) < FIX_ZERO(0.0));
		return mirror;
	}
	else
	{
...
float3 tex2Daa_subpixel_weights_only(Texture2D tex, SamplerState sam,
	const float2 tex_uv, const float2x2 pixel_to_tex_uv)
{
	...
	const float color_g = tex2D_linearize(tex, sam, tex_uv).g;
	const float color_r = tex2D_linearize(tex, sam, tex_uv + aa_r_offset_uv_offset).r;
	const float color_b = tex2D_linearize(tex, sam, tex_uv - aa_r_offset_uv_offset).b;
...
float3 tex2Daa4x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample0 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset0).rgb;
	const float3 sample1 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset1).rgb;
	const float3 sample2 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample3 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
...
float3 tex2Daa5x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample0 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset0).rgb;
	const float3 sample1 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset1).rgb;
	const float3 sample2 = tex2Daa_tiled_linearize(tex, sam, tex_uv).rgb;
	const float3 sample3 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample4 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (w0 * sample0 + w1 * sample1 +
		w2 * sample2 + w3 * sample3 + w4 * sample4);
}

float3 tex2Daa6x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample0 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset0).rgb;
	const float3 sample1 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset1).rgb;
	const float3 sample2 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset2).rgb;
	const float3 sample3 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset2).rgb;
	const float3 sample4 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample5 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (w0 * sample0 + w1 * sample1 + w2 * sample2 +
		w3 * sample3 + w4 * sample4 + w5 * sample5);
}

float3 tex2Daa7x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample1 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset1).rgb;
	const float3 sample2 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset2).rgb;
	const float3 sample3 = tex2Daa_tiled_linearize(tex, sam, tex_uv).rgb;
	const float3 sample4 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset2).rgb;
	const float3 sample5 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample6 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (
...
float3 tex2Daa8x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample1 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset1).rgb;
	const float3 sample2 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset2).rgb;
	const float3 sample3 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset3).rgb;
	const float3 sample4 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset3).rgb;
	const float3 sample5 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset2).rgb;
	const float3 sample6 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample7 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (
		w0 * sample0 + w1 * sample1 + w2 * sample2 + w3 * sample3 +
...
float3 tex2Daa12x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample2 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset2).rgb;
	const float3 sample3 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset3).rgb;
	const float3 sample4 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset4).rgb;
	const float3 sample5 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset5).rgb;
	const float3 sample6 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset5).rgb;
	const float3 sample7 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset4).rgb;
	const float3 sample8 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset3).rgb;
	const float3 sample9 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset2).rgb;
	const float3 sample10 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample11 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (
		w0 * sample0 + w1 * sample1 + w2 * sample2 + w3 * sample3 +
		w4 * sample4 + w5 * sample5 + w6 * sample6 + w7 * sample7 +
...
float3 tex2Daa16x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample4 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset4).rgb;
	const float3 sample5 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset5).rgb;
	const float3 sample6 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset6).rgb;
	const float3 sample7 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset7).rgb;
	const float3 sample8 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset7).rgb;
	const float3 sample9 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset6).rgb;
	const float3 sample10 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset5).rgb;
	const float3 sample11 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset4).rgb;
	const float3 sample12 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset3).rgb;
	const float3 sample13 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset2).rgb;
	const float3 sample14 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample15 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (
		w0 * sample0 + w1 * sample1 + w2 * sample2 + w3 * sample3 +
		w4 * sample4 + w5 * sample5 + w6 * sample6 + w7 * sample7 +
...
float3 tex2Daa20x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample6 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset6).rgb;
	const float3 sample7 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset7).rgb;
	const float3 sample8 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset8).rgb;
	const float3 sample9 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset9).rgb;
	const float3 sample10 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset9).rgb;
	const float3 sample11 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset8).rgb;
	const float3 sample12 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset7).rgb;
	const float3 sample13 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset6).rgb;
	const float3 sample14 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset5).rgb;
	const float3 sample15 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset4).rgb;
	const float3 sample16 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset3).rgb;
	const float3 sample17 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset2).rgb;
	const float3 sample18 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample19 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (
		w0 * sample0 + w1 * sample1 + w2 * sample2 + w3 * sample3 +
		w4 * sample4 + w5 * sample5 + w6 * sample6 + w7 * sample7 +
...
float3 tex2Daa24x(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	const float3 sample8 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset8).rgb;
	const float3 sample9 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset9).rgb;
	const float3 sample10 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset10).rgb;
	const float3 sample11 = tex2Daa_tiled_linearize(tex, sam, tex_uv + uv_offset11).rgb;
	const float3 sample12 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset11).rgb;
	const float3 sample13 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset10).rgb;
	const float3 sample14 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset9).rgb;
	const float3 sample15 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset8).rgb;
	const float3 sample16 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset7).rgb;
	const float3 sample17 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset6).rgb;
	const float3 sample18 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset5).rgb;
	const float3 sample19 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset4).rgb;
	const float3 sample20 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset3).rgb;
	const float3 sample21 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset2).rgb;
	const float3 sample22 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset1).rgb;
	const float3 sample23 = tex2Daa_tiled_linearize(tex, sam, tex_uv - uv_offset0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (
		w0 * sample0 + w1 * sample1 + w2 * sample2 + w3 * sample3 +
		w4 * sample4 + w5 * sample5 + w6 * sample6 + w7 * sample7 +
...
float3 tex2Daa_debug_16x_regular(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	//  Load samples, linearizing if necessary, etc.:
	const float3 sample0 = tex2Daa_tiled_linearize(tex, sam, sample0_uv).rgb;
	const float3 sample1 = tex2Daa_tiled_linearize(tex, sam, sample0_uv + uv_step_x).rgb;
	const float3 sample2 = tex2Daa_tiled_linearize(tex, sam, sample0_uv + uv_step_x * 2.0).rgb;
	const float3 sample3 = tex2Daa_tiled_linearize(tex, sam, sample0_uv + uv_step_x * 3.0).rgb;
	const float3 sample4 = tex2Daa_tiled_linearize(tex, sam, sample4_uv).rgb;
	const float3 sample5 = tex2Daa_tiled_linearize(tex, sam, sample4_uv + uv_step_x).rgb;
	const float3 sample6 = tex2Daa_tiled_linearize(tex, sam, sample4_uv + uv_step_x * 2.0).rgb;
	const float3 sample7 = tex2Daa_tiled_linearize(tex, sam, sample4_uv + uv_step_x * 3.0).rgb;
	const float3 sample8 = tex2Daa_tiled_linearize(tex, sam, sample8_uv).rgb;
	const float3 sample9 = tex2Daa_tiled_linearize(tex, sam, sample8_uv + uv_step_x).rgb;
	const float3 sample10 = tex2Daa_tiled_linearize(tex, sam, sample8_uv + uv_step_x * 2.0).rgb;
	const float3 sample11 = tex2Daa_tiled_linearize(tex, sam, sample8_uv + uv_step_x * 3.0).rgb;
	const float3 sample12 = tex2Daa_tiled_linearize(tex, sam, sample12_uv).rgb;
	const float3 sample13 = tex2Daa_tiled_linearize(tex, sam, sample12_uv + uv_step_x).rgb;
	const float3 sample14 = tex2Daa_tiled_linearize(tex, sam, sample12_uv + uv_step_x * 2.0).rgb;
	const float3 sample15 = tex2Daa_tiled_linearize(tex, sam, sample12_uv + uv_step_x * 3.0).rgb;
	//  Sum weighted samples (weight sum must equal 1.0 for each channel):
	return w_sum_inv * (
		w0 * sample0 + w1 * sample1 + w2 * sample2 + w3 * sample3 +
		w4 * sample4 + w5 * sample5 + w6 * sample6 + w7 * sample7 +
...
float3 tex2Daa_debug_dynamic(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	...
	for(int i = 0; i < grid_size; ++i)
	{
		...
		for(int j = 0; j < grid_size; ++j)
		{
			...
			sum += weights[i*grid_size + j] *
				tex2Daa_tiled_linearize(tex, sam, sample_uv).rgb;
		}
	}
...
float3 tex2Daa(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2x2 pixel_to_tex_uv, const float frame)
{
	//  Statically switch between antialiasing modes/levels:
	return aa_level < 0.5 ? tex2D_linearize(tex, sam, tex_uv).rgb :
		aa_level < 3.5 ? tex2Daa_subpixel_weights_only(
			tex, sam, tex_uv, pixel_to_tex_uv) :
		aa_level < 4.5 ? tex2Daa4x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 5.5 ? tex2Daa5x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 6.5 ? tex2Daa6x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 7.5 ? tex2Daa7x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 11.5 ? tex2Daa8x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 15.5 ? tex2Daa12x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 19.5 ? tex2Daa16x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 23.5 ? tex2Daa20x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 253.5 ? tex2Daa24x(tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		aa_level < 254.5 ? tex2Daa_debug_16x_regular(
			tex, sam, tex_uv, pixel_to_tex_uv, frame) :
		tex2Daa_debug_dynamic(tex, sam, tex_uv, pixel_to_tex_uv, frame);
}
