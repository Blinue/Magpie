...
float3 scanline_gaussian_integral_contrib(const float3 dist,
	...
	const float3 ph_offset = pixel_height * 0.5;
...
float3 scanline_gaussian_sampled_contrib(const float3 dist, const float3 color,
	...
	if (beam_antialias_level > 0.5) {
		...
		const float3 sample_offset = pixel_height / 3.0;
...
float3 get_scanline_color(Texture2D tex, SamplerState sam, const float2 scanline_uv,
	...
	const float3 color1 = tex.Sample(sam, scanline_uv).rgb;
...
float3 sample_single_scanline_horizontal(Texture2D tex, SamplerState sam,
	...
	const float4 final_weights = weights / dot(weights, float4(1, 1, 1, 1));
	...
	return get_scanline_color(
		tex, sam, prev_texel_hor_uv, uv_step_x, final_weights);
}

float3 sample_rgb_scanline_horizontal(Texture2D tex, SamplerState sam,
	const float2 tex_uv, const float2 texture_size,
	const float2 texture_size_inv) {
	//  TODO: Add function requirements.
	...
	if (beam_misconvergence) {
		...
		const float3 sample_r = sample_single_scanline_horizontal(
			tex, sam, scanline_uv_r, texture_size, texture_size_inv);
		const float3 sample_g = sample_single_scanline_horizontal(
			tex, sam, scanline_uv_g, texture_size, texture_size_inv);
		const float3 sample_b = sample_single_scanline_horizontal(
			tex, sam, scanline_uv_b, texture_size, texture_size_inv);
		return float3(sample_r.r, sample_g.g, sample_b.b);
	} else {
		return sample_single_scanline_horizontal(tex, sam, tex_uv, texture_size,
			texture_size_inv);
	}
}
...
float2 get_last_scanline_uv(const float2 tex_uv, const float2 texture_size,
	...
	const float frame_count_, out float dist) {
	...
	const float field_offset = floor(il_step_multiple.y * 0.75) *
		fmod(frame_count_ + float(interlace_bff), 2.0);
	const float2 curr_texel = tex_uv * texture_size;
	//  Use under_half to fix a rounding bug right around exact texel locations.
	const float2 prev_texel_num = floor(curr_texel - under_half);
	...
	const float2 scanline_texel = scanline_texel_num + 0.5;
