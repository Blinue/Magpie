...
inline float3 tex2DblurNfast(Texture2D tex, SamplerState sam, const float2 tex_uv,
	const float2 dxdy, const float sigma)
{
	...
	//  ate 10% of FPS across all codepaths, so it wasn't worth it.
		...
		{
			return tex2Dblur9fast(tex, sam, tex_uv, dxdy, sigma);
		}
		else if(sigma <= blur17_std_dev)
		{
			return tex2Dblur17fast(tex, sam, tex_uv, dxdy, sigma);
		}
		else if(sigma <= blur25_std_dev)
		{
			return tex2Dblur25fast(tex, sam, tex_uv, dxdy, sigma);
		}
		else if(sigma <= blur31_std_dev)
		{
			return tex2Dblur31fast(tex, sam, tex_uv, dxdy, sigma);
		}
		else
		{
			return tex2Dblur43fast(tex, sam, tex_uv, dxdy, sigma);
		}
	#else
