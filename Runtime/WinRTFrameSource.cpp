...
bool WinRTFrameSource::Initialize() {
    ...
    if (FAILED(hr)) {
        SPDLOG_LOGGER_CRITICAL(_logger, fmt::sprintf("DwmGetWindowAttribute 失败\n\tHRESULT：0x%X", hr));
        return false;
    }
    ...
    try {
        ...
        if (!winrt::ApiInformation::IsTypePresent(L"Windows.Graphics.Capture.GraphicsCaptureSession")) {
            SPDLOG_LOGGER_CRITICAL(_logger, "不存在 GraphicsCaptureSession API");
            App::SetErrorMsg(ErrorMessages::WINRT);
            return false;
        }
        if (!winrt::GraphicsCaptureSession::IsSupported()) {
            SPDLOG_LOGGER_CRITICAL(_logger, "当前不支持 WinRT 捕获");
            App::SetErrorMsg(ErrorMessages::WINRT);
            return false;
        }
        ...
        if (FAILED(hr)) {
            SPDLOG_LOGGER_CRITICAL(_logger, "创建 IDirect3DDevice 失败");
            App::SetErrorMsg(ErrorMessages::WINRT);
            return false;
        }
        ...
        if (!interop) {
            SPDLOG_LOGGER_CRITICAL(_logger, "获取 IGraphicsCaptureItemInterop 失败");
            App::SetErrorMsg(ErrorMessages::WINRT);
            return false;
        }
        ...
        if (FAILED(hr)) {
            SPDLOG_LOGGER_CRITICAL(_logger, "创建 GraphicsCaptureItem 失败");
            App::SetErrorMsg(ErrorMessages::WINRT);
            return false;
        }
        ...
        if (winrt::ApiInformation::IsPropertyPresent(
			...
			SPDLOG_LOGGER_INFO(_logger, "当前系统无 IsCursorCaptureEnabled API");
		}

        _captureSession.StartCapture();
    } catch (const winrt::hresult_error& e) {
        SPDLOG_LOGGER_CRITICAL(_logger, fmt::format("初始化 WinRT 失败：{}", Utils::UTF16ToUTF8(e.message())));
        App::SetErrorMsg(ErrorMessages::WINRT);
        return false;
    }
    ...
    if (FAILED(hr)) {
        SPDLOG_LOGGER_CRITICAL(_logger, "创建纹理失败");
        return false;
    }

    SPDLOG_LOGGER_INFO(_logger, "WinRTFrameSource 初始化完成");
    return true;
}
...
bool WinRTFrameSource::Update() {
	...
	if (FAILED(hr)) {
		SPDLOG_LOGGER_ERROR(_logger, "从获取 IDirect3DSurface 获取 ID3D11Texture2D 失败");
		return false;
	}
