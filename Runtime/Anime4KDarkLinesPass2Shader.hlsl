...
#define get(pos) (uncompressTan(SampleInputChecked(1, pos).x))
...
D2D_PS_ENTRY(main) {
	...
	return float4(compressTan(min(SampleInputCur(0).x - lumGaussian(Coord(1).xy, float2(0, Coord(1).w)), 0.0)), 0, 0, 1);
}
