...
D2D_PS_ENTRY(main) {
	...
	// [ml, mc, mr]
	// [bl, bc, br]
	float4 tl1 = uncompressLinear(SampleInput(0, leftTop1), -1, 4);
	float4 ml1 = uncompressLinear(SampleInput(0, float2(leftTop1.x, Coord(0).y)), -1, 4);
	float4 bl1 = uncompressLinear(SampleInput(0, float2(leftTop1.x, rightBottom1.y)), -1, 4);
	float4 tc1 = uncompressLinear(SampleInput(0, float2(Coord(0).x, leftTop1.y)), -1, 4);
	float4 mc1 = uncompressLinear(SampleInputCur(0), -1, 4);
	float4 bc1 = uncompressLinear(SampleInput(0, float2(Coord(0).x, rightBottom1.y)), -1, 4);
	float4 tr1 = uncompressLinear(SampleInput(0, float2(rightBottom1.x, leftTop1.y)), -1, 4);
	float4 mr1 = uncompressLinear(SampleInput(0, float2(rightBottom1.x, Coord(0).y)), -1, 4);
	float4 br1 = uncompressLinear(SampleInput(0, rightBottom1), -1, 4);

	float2 leftTop2 = GetCheckedOffPos(1, float2(-1, -1));
	float2 rightBottom2 = GetCheckedOffPos(1, float2(1, 1));

	float4 tl2 = uncompressLinear(SampleInput(1, leftTop2), -1, 4);
	float4 ml2 = uncompressLinear(SampleInput(1, float2(leftTop2.x, Coord(1).y)), -1, 4);
	float4 bl2 = uncompressLinear(SampleInput(1, float2(leftTop2.x, rightBottom2.y)), -1, 4);
	float4 tc2 = uncompressLinear(SampleInput(1, float2(Coord(1).x, leftTop2.y)), -1, 4);
	float4 mc2 = uncompressLinear(SampleInputCur(1), -1, 4);
	float4 bc2 = uncompressLinear(SampleInput(1, float2(Coord(1).x, rightBottom2.y)), -1, 4);
	float4 tr2 = uncompressLinear(SampleInput(1, float2(rightBottom2.x, leftTop2.y)), -1, 4);
	float4 mr2 = uncompressLinear(SampleInput(1, float2(rightBottom2.x, Coord(1).y)), -1, 4);
	float4 br2 = uncompressLinear(SampleInput(1, rightBottom2), -1, 4);
	...
	return compressLinear(c1234, -1, 4.5);
}
