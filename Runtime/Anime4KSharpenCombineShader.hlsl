...
float get1(float2 pos) {
	float2 f = frac(pos / coord1.zw);
	
	int2 i = int2(f * 2);
	float l = Uncompress1(D2DSampleInput(1, pos + (float2(0.5, 0.5) - f) * coord1.zw)[i.y * 2 + i.x]);
...
float getLuma() {
	if (curveHeight < 0.01) {
		return get1(coord1.xy);
	}

	float2 maxCoord = float2((srcSize.x - 1) * coord1.z, (srcSize.y - 1) * coord1.w);


	float left1X = max(0, coord1.x - coord1.z);
	float left2X = max(0, left1X - coord1.z);
	float left3X = max(0, left2X - coord1.z);
	float right1X = min(maxCoord.x, coord1.x + coord1.z);
	float right2X = min(maxCoord.x, right1X + coord1.z);
	float right3X = min(maxCoord.x, right2X + coord1.z);
	float top1Y = max(0, coord1.y - coord1.w);
	float top2Y = max(0, top1Y - coord1.w);
	float top3Y = max(0, top2Y - coord1.w);
	float bottom1Y = min(maxCoord.y, coord1.y + coord1.w);
	float bottom2Y = min(maxCoord.y, bottom1Y + coord1.w);
	float bottom3Y = min(maxCoord.y, bottom2Y + coord1.w);
	...
	float c[25] = {
		...
		get1(float2(coord1.x, top1Y)),	// c2
		get1(float2(right1X, top1Y)),	// c3
		get1(float2(left1X, coord1.y)),	// c4
		get1(float2(right1X, coord1.y)),	// c5
		get1(float2(left1X, bottom1Y)),	// c6
		get1(float2(coord1.x, bottom1Y)),	// c7
		get1(float2(right1X, bottom1Y)),	// c8
		get1(float2(coord1.x, top2Y)),	// c9
		get1(float2(left2X, coord1.y)),	// c10
		get1(float2(right2X, coord1.y)),	// c11
		get1(float2(coord1.x, bottom2Y)),	// c12
		get1(float2(coord1.x, bottom3Y)),	// c13
		get1(float2(right1X, bottom2Y)),	// c14
		get1(float2(left1X, bottom2Y)),	// c15
		get1(float2(right3X, coord1.y)),	// c16
		get1(float2(right2X, bottom1Y)),	// c17
		get1(float2(right2X, top1Y)),	// c18
		get1(float2(left3X, coord1.y)),	// c19
		get1(float2(left2X, bottom1Y)),	// c20
		get1(float2(left2X, top1Y)),		// c21
		get1(float2(coord1.x, top3Y)),	// c22
