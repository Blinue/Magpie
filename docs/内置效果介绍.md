...
* Bicubic：双立方（双三次）插值算法
  * 输出尺寸：取决于缩放选项
  * 参数
    * paramB：过滤参数 B。此项过大将模糊
    * paramC：过滤参数 C。此项过大将产生振铃
...
* CRT_Easymode：易于配置的轻量级 CRT 着色器
  ...
  * 参数
    ...
    * maskSize
    * scanlineStrength
    * scanlineBeamWidthMin
    * scanlineBeamWidthMax
    * scanlineBrightMin
    * scanlineBrightMax
    * scanlineCutoff
    * gammaInput
...
* ImageAdjustment：图像参数调整
  * 输出尺寸：和输入相同
  * 参数
    * targetGamma：和 monitorGamma 搭配使用以调节 gamma，此参数指定目标 gamma 值
    * monitorGamma：当前屏幕的 gamma 值
    * saturation：色度
    * luminance：HSV 空间的明度
    * contrast：对比度
    * brightBoost：亮度调整
...
* Lanczos：使用 Lanczos 算法缩放输入。
  * 输出尺寸：取决于缩放选项
  * 参数
    * ARStrength：抗震铃强度。值越大抗震铃效果越好，但图像越模糊
...
* LumaSharpen：reshade 中流行的锐化效果。
  ...
  * 参数
    * sharpStrength：锐化强度
    * sharpClamp：抗振铃强度
    * pattern：滤波器类型，支持 4 种滤波器
    * offsetBias：滤波器参数的偏移量
