...
class Anime4KUpscaleDeblurOutputTransform : public DrawTransformBase {
private:
    Anime4KUpscaleDeblurOutputTransform() {}
public:
    static HRESULT Create(_In_ ID2D1EffectContext* d2dEC, _Outptr_ Anime4KUpscaleDeblurOutputTransform** ppOutput) {
        *ppOutput = nullptr;
        ...
        *ppOutput = new Anime4KUpscaleDeblurOutputTransform();
    ...
    IFACEMETHODIMP SetDrawInfo(ID2D1DrawInfo* pDrawInfo) {
        _drawInfo = pDrawInfo;
        return pDrawInfo->SetPixelShader(GUID_MAGPIE_ANIME4K_UPSCALE_DEBLUR_OUTPUT_SHADER);
    }
