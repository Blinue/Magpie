...
#include "Anime4KUpscaleOutputTransform.h"
...
public:
    IFACEMETHODIMP Initialize(
        ...
        hr = Anime4KUpscaleOutputTransform::Create(pEffectContext, &_convOutputTransform);
        if (FAILED(hr)) {
            return hr;
        }
        ...
        hr = pTransformGraph->AddNode(_convOutputTransform.Get());
        if (FAILED(hr)) {
            return hr;
        }
        ...
        hr = pTransformGraph->ConnectToEffectInput(0, _convOutputTransform.Get(), 0);
        if (FAILED(hr)) {
            return hr;
        }
        hr = pTransformGraph->ConnectNode(_convReduceTransform.Get(), _convOutputTransform.Get(), 1);
        if (FAILED(hr)) {
            return hr;
        }

        hr = pTransformGraph->SetOutputNode(_convOutputTransform.Get());
        if (FAILED(hr)) {
            return hr;
        }
    ...
    static HRESULT Register(_In_ ID2D1Factory1* pFactory) {
        HRESULT hr = pFactory->RegisterEffectFromString(CLSID_MAGIPE_ANIME4K_UPSCALE_EFFECT, XML(
            <?xml version='1.0'?>
                <Effect>
                <!--System Properties-->
    ...
    static HRESULT CALLBACK CreateEffect(_Outptr_ IUnknown** ppEffectImpl) {
        ...
        *ppEffectImpl = static_cast<ID2D1EffectImpl*>(new Anime4KUpscaleEffect());
...
private:
    ...
    ComPtr<Anime4KUpscaleOutputTransform> _convOutputTransform = nullptr;
};
