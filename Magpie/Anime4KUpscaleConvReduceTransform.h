...
class Anime4KUpscaleConvReduceTransform : public DrawTransformBase {
private:
    Anime4KUpscaleConvReduceTransform() {}
public:
    static HRESULT Create(_In_ ID2D1EffectContext* d2dEC, _Outptr_ Anime4KUpscaleConvReduceTransform** ppOutput) {
        *ppOutput = nullptr;
        ...
        *ppOutput = new Anime4KUpscaleConvReduceTransform();
    ...
    IFACEMETHODIMP SetDrawInfo(ID2D1DrawInfo* pDrawInfo) {
        _drawInfo = pDrawInfo;
        return pDrawInfo->SetPixelShader(GUID_MAGPIE_ANIME4K_UPSCALE_CONV_REDUCE_SHADER);
    }
